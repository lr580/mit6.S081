...
运行进程数，同理，去 `proc.c` 定义一个函数。在 `proc.h` 定义了 `struct proc`，有枚举成员 `state`，若枚举值不为 `UNUSED`，则空闲(虽然不懂为什么逻辑反着来的，但是文档就这么说的)。其中，`proc.c` 的 `struct proc proc[NPROC]` 维护了所有运行中的进程，所以遍历这个数组即可(也可以用指针遍历)。可以不用锁。
